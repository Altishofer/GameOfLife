...
public class SingletonGUI extends JFrame {
    ...
    private final TextFieldWithPrompt aTextField1 = new TextFieldWithPrompt("Player_1", 8, 18);
    private final TextFieldWithPrompt aTextField2 = new TextFieldWithPrompt("Player_2", 8, 18);
    private final TextFieldWithPrompt aTextField3 = new TextFieldWithPrompt("Enter Even Board Size (10-20)", 8, 27);
    private final JButton aConfirmButton = new JButton("Confirm");
    private final JButton aRedButton1 = GuiUtils.getButton(aIconSize, ColorType.RED.toColor());
    private final JButton aRedButton2 = GuiUtils.getButton(aIconSize, ColorType.RED.toColor());
    private final JButton aBlueButton1 = GuiUtils.getButton(aIconSize, ColorType.BLUE.toColor());
    private final JButton aBlueButton2 = GuiUtils.getButton(aIconSize, ColorType.BLUE.toColor());
    private final Player aPlayer1;
    private final Player aPlayer2;
    private final JSplitPane aSplitPaneChartBoard;
    ...
    private boolean someoneHasLost(){
        if (!aGame.initOver()){return false;}
        ...
        else if (aGrid.getCellCount(aPlayer1.getPlayerColor()) == 0){
            setMessage(aPlayer1.getPlayerName() + " has lost! -> " + aPlayer2.getPlayerName().toUpperCase() + " wins");
            return true;
        }
        else if (aGrid.getCellCount(aPlayer2.getPlayerColor()) == 0){
    ...
    private void checkIfLost(){
        if (someoneHasLost()) {disableAllFinished();}
    }
    ...
    private void showGrid() {
        for (int i = 0; i < aSize; i++) {
            for (int j = 0; j < aSize; j++) {
                Color tmp = aGrid.getColor(i, j).toColor();
